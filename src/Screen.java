...
public class Screen extends JPanel implements Runnable {
	...
	public void define() {
		...
		for(int i=0; i<mobs.length; i++) {
			mobs[i] = new Mob(); //calls the Mob method to create a mob
		}
	}
	...
	public int spawnTime = 00, spawnFrame = 0; //spawnTime = Rate of spawn for mobs //spawnFrame = wait until 1st spawn
	public void mobSpawner() {                 //mob spawner to spawn the mobs
		if(spawnFrame >= spawnTime) {
			for(int i=0; i<mobs.length;i++) {
				if(!mobs[i].inGame) {            //if the mob is not in the game
					if(!mobs[i].deleted()) { //keeps the mob from respawining if deleted`
	...
	public void run() {
		while(true) {
			if(!isFirst && health > 0 && !isWin) {
				...
				mobSpawner();
				for(int i=0;i<mobs.length;i++) {
					if(mobs[i].inGame) { //checks if the mob is in the game
