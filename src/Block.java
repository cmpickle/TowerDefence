...
public class Block extends Rectangle {
	...
	public void physic() {
		if(shotMob != -1 && towerSquare.intersects(Screen.mobs[shotMob])) { //if the mob intersects towerSquare
		...
		if(!shooting) {
			...
			if(airID == Value.airTowerLazer) { //if an airID has a Lazer Tower add other towers to this if statement
				for(int i=0; i<Screen.mobs.length;i++) { //do this for all mobs
					if(Screen.mobs[i].inGame) { //mobs have to be in game
						if(towerSquare.intersects(Screen.mobs[i])) { //if the mob intersects towerSquare
							shooting = true;  //lets tower shoot mob
							shotMob = i;
						}
					}
